...
const EditProduct = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      if (error) {
        console.error("Lỗi khi tải sản phẩm (ID: " + id + "):", error.message);
    ...
    // [SỬA LỖI CUỐI CÙNG]
    if (isCreating) {
      // 1. CHẾ ĐỘ THÊM MỚI: Reset form
      setProduct(initialProductState);
    } else if (id && id !== "undefined") {
      // 2. CHẾ ĐỘ SỬA: Chỉ fetch NẾU id tồn tại VÀ KHÔNG PHẢI chuỗi "undefined"
      fetchProduct();
    }
    // 3. Nếu id là undefined (đang tải) hoặc là chuỗi "undefined", không làm gì cả.
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isCreating) {
        ...
        // Chỉ submit nếu id hợp lệ (phòng trường hợp)
        if (!id || id === "undefined") {
